...
export const PaymentDetail: FC<PaymentDetailProps> = (props) => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      await fd.append("timePaymentAt", data.paymentAt ? data.paymentAt.toLocaleTimeString() : now.toLocaleTimeString());
      ...
      if (result && result.data && result.data.reference) {
        ...
        await router.push('/virtuals/orders/success');

        if (props.order.invoice.paymentMethodType === PaymentMethodType.SHOPDITPAY_AIRPAY ||
          props.order.invoice.paymentMethodType === PaymentMethodType.SHOPDITPAY_BAYBANK ||
          props.order.invoice.paymentMethodType === PaymentMethodType.SHOPDITPAY_BBL ||
          props.order.invoice.paymentMethodType === PaymentMethodType.SHOPDITPAY_CREDIT_CARD ||
          props.order.invoice.paymentMethodType === PaymentMethodType.SHOPDITPAY_LINEPAY ||
          props.order.invoice.paymentMethodType === PaymentMethodType.SHOPDITPAY_SCB_EASY ||
          props.order.invoice.paymentMethodType === PaymentMethodType.SHOPDITPAY_TRUEMONEY
        ) {
  ...
  return (
      ...
      >
        <Box p={isDesktop ? '0' : '16px'}>
          {isDesktop && <Typography component="h1" variant="h1" fontWeight="300">
            {t("title")}
          </Typography>}
              ...
              >
                ...
                <Box>
                  <Typography variant="h4" fontWeight="light">
                    {
                      props.order.invoice.merchantBankAccountPaymentMethod.bank
                        .name
                    }
                  </Typography>
          ...
          {props.order.invoice?.paymentMethodType === PaymentMethodType.OMISE &&
            props.order.invoice?.customerCreditCard && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">ชำระด้วยบัตรเครดิต</Typography>
                <Box
                  display="flex"
                  justifyContent="space-between"
                >
                  <Box>
                    <Typography variant="h3">{props.order.invoice?.customerCreditCard?.cardName}</Typography>
                  </Box>
                  <Box>
                    <Typography variant="h3">*** {props.order.invoice?.customerCreditCard?.lastNumber}</Typography>
                  </Box>
                </Box>
              </Box>
            )}
          {props.order.invoice?.paymentMethodType === PaymentMethodType.SHOPDITPAY_AIRPAY &&
            props.order.invoice?.merchantShopditPaymentMethod && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">Shopee Pay</Typography>
              </Box>
            )}
          {props.order.invoice?.paymentMethodType === PaymentMethodType.SHOPDITPAY_BAYBANK &&
            props.order.invoice?.merchantShopditPaymentMethod && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">KMA (กรุงศรีโมบายแอป)</Typography>
              </Box>
            )}
          {props.order.invoice?.paymentMethodType === PaymentMethodType.SHOPDITPAY_BBL &&
            props.order.invoice?.merchantShopditPaymentMethod && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">Bualuang mBanking</Typography>
              </Box>
            )}
          {props.order.invoice?.paymentMethodType === PaymentMethodType.SHOPDITPAY_CREDIT_CARD &&
            props.order.invoice?.merchantShopditPaymentMethod && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">บัตรเครดิต</Typography>
              </Box>
            )}
          {props.order.invoice?.paymentMethodType === PaymentMethodType.SHOPDITPAY_LINEPAY &&
            props.order.invoice?.merchantShopditPaymentMethod && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">Line Pay</Typography>
              </Box>
            )}
          {props.order.invoice?.paymentMethodType === PaymentMethodType.SHOPDITPAY_SCB_EASY &&
            props.order.invoice?.merchantShopditPaymentMethod && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">SCB EASY</Typography>
              </Box>
            )}
          {props.order.invoice?.paymentMethodType === PaymentMethodType.SHOPDITPAY_TRUEMONEY &&
            props.order.invoice?.merchantShopditPaymentMethod && (
              <Box
                ...
                borderColor="grey.100"
              >
                <Typography variant="h3" pb="16px">True Money</Typography>
              </Box>
            )}
        </Box>
        <Box
                  ...
                  <Box borderTop="1px solid" py="32px" borderColor="grey.100">
                    <Box display="flex" justifyContent="space-between">
                      ...
                      <Typography variant="h4">
                        {intl.formatNumber(props.order?.invoice?.totalPrice ?? 0)} ฿
                      </Typography>
                    </Box>
                  </Box>
            ...
            >
              ...
              >
                ...
                <FormControl sx={{ flex: 1 }}>
                  ...
                  <Box borderTop="1px solid" borderColor="grey.100" my="24px" />
                  <Button
                    variant="outlined"
                    fullWidth
                    sx={{ py: "16px", borderRadius: "8px" }}
                    disabled={
                      (onLoading &&
                        (watch("file") === null || watch("file") === undefined))
                    }
                    onClick={() => {
