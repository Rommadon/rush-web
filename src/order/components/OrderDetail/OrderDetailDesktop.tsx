...
export const OrderDetailDesktop: FC<OrderDetailDesktopProps> = (props) => {
  ...
  return (
    <DefaultLayout
      ...
      titleMeta={`คำสั่งซื้อ ${props.order.number}`}
    >
      {/* <SlipUploadModal
        banks={banks}
        open={slipOpen}
        ...
        >
            ...
            <Box>
              ...
              <Typography variant="h4" fontWeight="light">
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.BANK_ACCOUNT && "โอนเงินผ่านธนาคาร"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.CASH && "เงินสด"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.OMISE && "บัตรเครดิต"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.PAYPAL && "Paypal"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.PROMPTPAY && "พร้อมเพย์"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.SHOPDITPAY_AIRPAY && "Shopee Pay"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.SHOPDITPAY_BAYBANK &&
                  "KMA (กรุงศรีโมบายแอป)"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.SHOPDITPAY_BBL && "Bualuang mBanking"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.SHOPDITPAY_CREDIT_CARD && "บัตรเครดิต"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.SHOPDITPAY_LINEPAY && "Line Pay"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.SHOPDITPAY_SCB_EASY && "SCB EASY"}
                {props.order.invoice.paymentMethodType ===
                  PaymentMethodType.SHOPDITPAY_TRUEMONEY && "Truemoney"}
              </Typography>
            </Box>
            <Box>
                  ...
                  <NextImage
                    ...
                    src={
                      orderItem.productItem?.product?.productImages?.find(
                        (image: ImageUpload) => image.order === 0
                      )?.imageUpload?.url ?? ""
                    }
                    width={140}
                    height={140}
          ...
          {props.order.status === "pendingPayment" && (
            <OrderPaymentDetail
              ...
              onUploadSlipClick={() => selectPushPage(props.type)}
            />
          )}
        ...
        <Box display="grid" mt="32px" gridTemplateColumns="2fr 1fr" gap="64px">
          ...
          <Box>
            <Box
              ...
              borderRadius="8px"
            >
              <Typography variant="h2">หมายเหตุถึงร้านค้า</Typography>
              <Box height="1px" bgcolor="grey.100" my="24px" />
              <Typography fontWeight="light">
                {[props.order.note]
                  .filter((string) => string?.length)
          ...
          >
            ...
            <Box display="flex" justifyContent="space-between" py="16px">
              <Typography variant="h4">ส่วนลดจากโค้ดส่วนลด</Typography>
      ...
      <Modal open={cancelByMobileOpen} onClose={closeCancelByMobile}>
        <Box
          sx={{
            position: "absolute",
            top: "50%",
