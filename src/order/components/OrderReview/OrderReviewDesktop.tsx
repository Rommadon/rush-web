...
import { DefaultLayout, ChevronDownIcon, useResource } from "src/core";
import {
  UseFormGetValues,
  UseFormHandleSubmit,
  UseFormRegister,
...
export const OrderReviewDesktop: FC<OrderReviewDesktopProps> = (props) => {
  ...
  const onSelectCoupon = async (selectedCoupon: CouponModel | null) => {
    ...
    const validateData = await resource.fetchResource(
      `coupon-public/${selectedCoupon?.code}/validate?amount=${cartPrice.totalPrice}`,
      {},
      null
    );
  ...
  useEffect(() => {
    ...
    if (props?.customerAddresses) {
      ...
      if (props?.customerAddresses && props?.customerAddresses.length > 0) {
        props.setValue("customerAddressId", props?.customerAddresses![0].id);
      }
    }
          ...
          >
            <Box>
                      ...
                      >
                        <Box display="flex" alignItems="center">
                          <Radio
                            size="small"
                            checked={
                              props.watch("customerAddressId") === address.id
                            }
                            onClick={() =>
                ...
                >
                  ...
                  <AccordionDetails>
                    <ShippingForm
                      ...
                      currentMerchantShipmentId={props.watch(
                        "merchantShipmentId"
                      )}
                ...
                >
                  ...
                  <AccordionDetails>
                    <PaymentOptionForm
                      ...
                      setValue={props?.setValue}
                      currentInvoiceAttributes={props.watch(
                        "invoiceAttributes"
                      )}
                      customerCreditCards={customerCreditCards}
                      onOpenCreateCreditCard={() =>
                        setCreditCardModalOpen(true)
                      }
                      merchantShopditPaymentMethods={
                        merchantShopditPaymentMethods
                      }
                      isActiveCreditCardPaymentMethod={
                        isActiveCreditCardPaymentMethod
                      }
                      totalPrice={
                        cartPrice?.totalPrice - discountPrice + shipmentPrice
                      }
                    />
                  </AccordionDetails>
                </Accordion>
              ...
              >
                <Accordion
                  ...
                  disableGutters
                  elevation={0}
                  onChange={() => setNoteExpanded(!noteExpanded)}
                >
                  <AccordionSummary
                    expandIcon={<ChevronDownIcon />}
                    sx={{ py: "16px" }}
            ...
            <Box>
              <OrderReviewSidebar
                ...
                currentOrderShipmentAttributes={props.watch(
                  "orderShipmentAttributes"
                )}
