...
export type NotificationListProps = OrderLayoutProps & {
  ...
  directNotificationBroadcasts: DirectNotificationModel[];
  directNotificationBroadcastMeta: any;
};
...
export const NotificationList: FC<NotificationListProps> = (props) => {
  ...
  const [directNotificationBroadcastsData, setDirectNotificationBroadcastsData] = useState<
  DirectNotificationModel[]
>(props.directNotificationBroadcasts || []);
  const [page, setPage] = useState<number>(1);
  const [pageBroadcast, setPageBroadcast] = useState<number>(1);
  ...
  const onFetchDataBroadcast = async () => {
    const fetchOrder = await resource.fetchResource(
      `direct-notification-public/broadcast?page=${page + 1}`,
      {},
      ""
    );
    setPageBroadcast(page + 1);
    setDirectNotificationBroadcastsData(
      directNotificationsData.concat(fetchOrder?.data?.data)
    );
  };
  ...
  useEffect(() => {
    if (props.directNotificationBroadcasts) {
      setDirectNotificationBroadcastsData(props.directNotificationBroadcasts);
    }
  }, [props.directNotificationBroadcasts]);
  ...
  const notificationType = (directNotification: DirectNotificationModel) => {
    switch (directNotification.detailType) {
      ...
      default:
        return directNotification?.groupNotification?.title || '-';
    }
  };
  ...
  const notificationMessageType = (
    ...
    switch (directNotification.detailType) {
      ...
      default:
        return directNotification?.message || '-';
    }
  };
        ...
        >
          {
            directNotificationsData && directNotificationsData.length > 0 ? (
              ...
              >
                {directNotificationsData?.map((directNotification) => (
                  <NotificationItem
          ...
          >
            {directNotificationBroadcastsData?.map((directNotification) => (
              <NotificationItem
                key={directNotification?.id}
                id={directNotification?.id}
                title={notificationType(directNotification)}
                src={
                  directNotification?.groupNotification?.imageUpload?.url || ""
                }
                body={notificationMessageType(directNotification)}
                createdAt={new Date(directNotification.createdAt).toString()}
                isAlready={isAlready ? isAlready : directNotification.isAlready}
