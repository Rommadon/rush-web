...
export const NotificationItem: FC<NotificationProps> = (props) => {
  ...
  const stringWithHyphens = (stringDate: string) => {
      ...
      <Box display="flex" width="100%" bgcolor={`${isAlready ? 'white' : 'grey.50'}`} p="16px" borderBottom="1px solid" borderColor="grey.100" onClick={() => updateIsAlready()}>
        ...
        <Box p="8px" flex="1">
          ...
          <Box mt="8px">
            ...
            {props.createdAt && (
              <Typography variant="h6" color="grey.200" fontWeight="light" py="8px" textAlign="right">
                {stringWithHyphens(intl.formatDateTime(new Date(props.createdAt)))}
              </Typography>
            )}
          </Box>
        </Box>
